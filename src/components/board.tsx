...
import { archiveAllCards, useGetCardsByBoardId } from '../api/card';
...
export const Board = ({ body }: BoardProps) => {
  ...
  if (columnsError && cardsError) {
    return (
      <div>
        error: {columnsError} {cardsError}
      </div>
    );
  }

  const handleArchiveAll = async () => {
    await archiveAllCards(body.id);
    refetchCards();
  };
    ...
    <BoardIdProvider boardId={body.id}>
      <div className="board">
        ...
        <div className="board-columns">
          {columns.map((column) => (
            <Column key={column.id} body={column} cards={cards} refetchCards={refetchCards} />
          ))}
        </div>
      </div>
    </BoardIdProvider>
